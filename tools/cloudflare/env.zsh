...
if [ -f ~/.cloudflare/cloudflare.cfg ]; then
    ...
    export R2_TFSTATE_KEY=$(grep r2_tfstate_key ~/.cloudflare/cloudflare.cfg | cut -d '=' -f2 | tr -d '[:space:]')
    export R2_TFSTATE_SECRET=$(grep r2_tfstate_secret ~/.cloudflare/cloudflare.cfg | cut -d '=' -f2 | tr -d '[:space:]')
fi
