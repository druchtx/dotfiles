...
---connects session save/load with tab workspace persistence in `utils.tabs`.
...
local function load_session(item)
  ...
  require("utils.tabs").set_session_project(item.dir)
...
function M.patch(persistence)
  ...
  persistence.current = function(opts)
    opts = opts or {}
    local session_dir = require("utils.tabs").session_project() or vim.fn.getcwd()
  ...
  persistence.load = function(opts)
    ...
    local ret = load(opts)
    require("utils.tabs").restore_session_state()
    return ret
  end
