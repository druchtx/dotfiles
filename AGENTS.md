...
Script entries are executed by `./bin/dfm` instead of being symlinked.
...
- `./bootstrap`: install base dependencies and run `./bin/dfm init`.
- `./bin/dfm init`: scan templates and update `.dotenv`.
- `./bin/dfm link`: apply manifest entries by creating symlinks and
  executing script entries.
- `./bin/dfm unlink`: remove symlinks and run script cleanup entries.
- `./bin/dfm status`: verify link health and detect conflicts.
- `./bin/dfm sync --force`: pull the repo, update Homebrew and mise tools, then relink.
- `./bin/dfm --help`: inspect available subcommands before extending the CLI.
...
- Run `./bin/dfm status` after manifest or symlink changes.
- Run `./bin/dfm link --force` only when you intend to overwrite conflicting local files.
- For Python script edits, use `python3 -m py_compile bin/dfm bin/remote-vsc`.
